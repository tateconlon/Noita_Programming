...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Player : MMFeedbacks
	{
		...
		public override float TotalDuration
		{
			get
			{
				// if (Mathf.Approximately(_cachedTotalDuration, 0f))
				// {
				// 	Debug.LogWarning($"{nameof(_cachedTotalDuration)} is zero. Call {nameof(RefreshCache)} on this {nameof(MMF_Player)}", this);
				// }
