...
namespace MoreMountains.TopDownEngine
{
	...
	public class DamageOnTouch : MMMonoBehaviour
	{
		...
		public UnityEvent<FEELHealth> HitDamageableEvent;
		...
		protected FEELHealth _colliderHealth;
		protected TopDownController _topDownController;
		protected Rigidbody2D _colliderTopDownController;  // ASSETMOD: changed from TopDownController to Rigidbody2D
		protected FEELHealth _health;
		...
		{
			_health = GetComponent<FEELHealth>();
		...
		{
			MinDamageCaused = newDamage;
			MaxDamageCaused = newDamage;
		}
		...
		protected virtual void Colliding(GameObject collider)
		{
			...
			_colliderHealth = collider.gameObject.MMGetComponentNoAlloc<FEELHealth>();
		...
		protected virtual void OnCollideWithDamageable(FEELHealth health)
		{
			if (!health.CanTakeDamageThisFrame())
			{
				//Debug.Log(health.name + " invinsible from " + name, health);
				return;
			}
			
			//Debug.Log(health.name + " hit by " + name, health);
			
			// if what we're colliding with is a TopDownController, we apply a knockback force
			_colliderTopDownController = health.gameObject.MMGetComponentNoAlloc<Rigidbody2D>();
		...
		protected virtual void ApplyKnockback(float damage, List<TypedDamage> typedDamages)
		{
			if (ShouldApplyKnockback(damage, typedDamages))
			{
				...
				{
					_colliderTopDownController.AddForce(_knockbackForce, KnockbackForceMode);
				}
			}
		}
		...
		protected virtual void ApplyKnockback2D()
		{
			switch (DamageCausedKnockbackDirection)
			{
				case KnockbackDirections.BasedOnSpeed:
					Vector3 totalVelocity = _colliderTopDownController.velocity + (Vector2)_velocity;
		...
		protected virtual void ApplyKnockback3D()
		{
			...
			{
				case KnockbackDirections.BasedOnSpeed:
					Vector3 totalVelocity = (Vector3)_colliderTopDownController.velocity + _velocity;
		...
		{
			//Debug.Log($"{name} hit non-damageable", this);
		...
		protected virtual void SelfDamage(float damage)
		{
			...
			{
				Vector2 totalVelocity = _colliderTopDownController.velocity + (Vector2)_velocity;
