...
namespace NodeCanvas.StateMachines
{
    ...
    public abstract class FSMNode : Node
    {
        ...
        // ASSETMOD: Added portRectTop support
...
#pragma warning restore EventHandlerLeakAnalyzer
            ...
            var portRectTop = new Rect(0, 0, 20, 20);

            portRectLeft.center = new Vector2(rect.x - 11, rect.center.y - 10);
            portRectRight.center = new Vector2(rect.xMax + 11, rect.center.y + 10);
            portRectBottom.center = new Vector2(rect.center.x - 10, rect.yMax + 11);
            portRectTop.center = new Vector2(rect.center.x + 10, rect.y - 11);
            ...
            if ( maxOutConnections != 0 ) {
                if ( fullDrawPass || drawCanvas.Overlaps(rect) ) {
                    ...
                    UnityEditor.EditorGUIUtility.AddCursorRect(portRectTop, UnityEditor.MouseCursor.ArrowPlus);
                    ...
                    GUI.DrawTexture(portRectTop, Editor.StyleSheet.arrowTop);
                    ...
                    if ( Editor.GraphEditorUtility.allowClick && e.type == EventType.MouseDown && e.button == 0 ) {
                        ...
                        if (portRectTop.Contains(e.mousePosition) ) {
                            clickedPort = new GUIPort(this, portRectTop.center);
                            dragDropMisses = 0;
                            e.Use();
                        }
                    }
            ...
            for ( var i = 0; i < outConnections.Count; i++ ) {
                ...
                if (Mathf.Abs(rect.center.x - targetPos.x) < 200)
                {
                    if (rect.center.y < targetPos.y - 50) {
                        sourcePos = portRectBottom.center;
                    }
                    
                    if (rect.center.y > targetPos.y + 50) {
                        sourcePos = portRectTop.center;
                    }
                }
        ...
        // ASSETMOD: Changed connection style to be straight lines
        //...
        Vector2 GetConnectedInPortPosition(Connection connection) {
            ...
            if (Mathf.Abs(sourcePos.x - thisPos.x) < 200)
            {
                if (sourcePos.y < thisPos.y - 50)
                {
                    return new Vector2(rect.center.x - 10, rect.y - 3);
                }
                
                if (sourcePos.y > thisPos.y + 50)
                {
                    return new Vector2(rect.center.x + 10, rect.yMax + 3);
                }
            }
            
            if (sourcePos.x <= thisPos.x)
            {
                return new Vector2(rect.xMin - 3, rect.center.y + 10);
            }
            if (sourcePos.x > thisPos.x)
            {
                return new Vector2(rect.xMax + 3, rect.center.y - 10);
            }
