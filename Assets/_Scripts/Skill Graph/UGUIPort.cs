...
public class UGUIPort : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerEnterHandler,
    IPointerExitHandler
{
    ...
    public void UpdateConnectionTransforms()
    {
        ...
        for (int i = 0; i < port.ConnectionCount; i++)
        {
            ...
            UiNodeBase otherSkillNode = graph.GetRuntimeNode(other.node);
            if (!otherSkillNode) Debug.LogWarning(other.node.name + " node not found", this);
            Transform port2 = otherSkillNode.GetPort(other.fieldName).transform;
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        ...
        else
        {
            if (port.IsConnected)
            {
                ...
                UiNodeBase otherSkillNode = graph.GetRuntimeNode(output.node);
                UGUIPort otherUGUIPort = otherSkillNode.GetPort(output.fieldName);
